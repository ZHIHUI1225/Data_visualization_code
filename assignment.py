#!/usr/bin/env python3
"""
Multi-Robot Task Assignment and Path Planning Pipeline

LOADED FILES:
- /root/workspace/data/Graph_{case}.json                    : Original Reeb graph from environment processing
- /root/workspace/data/environment_{case}.json              : Environment data with obstacles and coordinate frames
- /root/workspace/config/config.py                          : System configuration (case, N, arc_range, start/goal)
- /root/workspace/data/Estimated_matrices_{case}.npz        : Generated matrices for optimization (Ec, El, Ad, Cr, Cl)
- /root/workspace/data/Normalization{N}_{case}.json         : Normalization parameters for integer programming
- /root/workspace/data/AssignmentResult{N}{case}.json       : Assignment results with waypoints and relay points

SAVED FILES:
- /root/workspace/data/Graph_new_{case}.json                : Rebuilt and optimized graph with coordinate frame metadata
- /root/workspace/data/Estimated_matrices_{case}.npz        : Generated constraint matrices for multi-robot coordination
- /root/workspace/data/Normalization{N}_{case}.json         : Normalization parameters for optimization scaling
- /root/workspace/data/AssignmentResult{N}{case}.json       : Integer programming solution with robot assignments
- /root/workspace/data/WayPointFlag{N}{case}.json          : Final waypoint sequence with turning direction flags
- /root/workspace/data/Reeb_graph_{case}.png                : Visualization of rebuilt graph with environment (steelblue nodes)
- /root/workspace/data/Assignment_Result{N}_{case}.png      : Visualization of assignment solution with path arrows

FUNCTION:
This is the main pipeline script that orchestrates the complete multi-robot task assignment process.
It integrates graph processing, matrix generation, optimization, and path planning into a cohesive workflow:

1. GRAPH REBUILDING: Loads original Reeb graph and rebuilds it with improved connectivity and start/goal integration
2. MATRIX GENERATION: Computes constraint matrices (Ec, El, Ad, Cr, Cl) for multi-robot coordination optimization
3. NORMALIZATION: Calculates scaling parameters to ensure numerical stability in integer programming
4. OPTIMIZATION: Solves integer programming problem to assign robots to graph arcs optimally
5. PATH EXTRACTION: Converts optimization results into executable waypoint sequences with turn directions
6. VISUALIZATION: Generates plots showing environment, graph, and assigned robot paths

Key Components:
- Ec, El, Ad: Geometric constraint matrices for robot coordination
- Cr, Cl: Turn constraint matrices for left/right turning decisions
- Waypoints: Sequential robot movement commands between graph nodes
- RelayPoints: Coordination points where robots must synchronize
- FlagB: Turn direction flags (-1=right, 1=left) for relay point navigation

The pipeline maintains coordinate frame consistency (world_pixel) throughout all processing stages
and generates executable commands for the e-puck2 robot behavior tree system.

Dependencies: Graph, Node, Environment, Rebuild_large, GenerateMatrix, normalization, IntegerProgramming
Author: Multi-Robot Pushing System
Usage: Run directly to execute complete assignment pipeline for configured case
"""
...
# Load configuration from config file
    ...
    # Save the rebuilt graph visualization
    rebuilt_graph_figure_name = f"/root/workspace/data/Reeb_graph_{case}.png"
    plt.savefig(rebuilt_graph_figure_name, dpi=300, bbox_inches='tight')
    print(f"Reeb graph figure saved to {rebuilt_graph_figure_name}")
    plt.show()
    ...
    ########## Integer programming solve ##########
        ...
        Visualization(reeb_graph_new,loaded_environment,Result_file="/root/workspace/data/" + f"AssignmentResult{N}"+case+".json",figure_name=f"/root/workspace/data/Assignment_Result{N}_{case}.png")
