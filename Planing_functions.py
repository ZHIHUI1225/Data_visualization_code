...
def Planning_error_withinSC(waypoints_file_path,Normalization_path,environment_file,safe_corridor,reeb_graph,phi0,Initial_Guess_file_path,Result_file,figure_file):
    ...
    for i in range(N-1):
        ...
        # objective = objective + al[i] * (arc_length + straight_length - length_min[i]) + 1*ac * curvature
        objective = objective + al[i] * (straight_length - length_min[i]) + 1*ac * curvature
    ...
    opts_setting = {
        'ipopt.max_iter': 6000,           # Increase iterations for complex constraints
    ...
    boundary_width = 3
    ax.plot([x_min, x_max], [y_min, y_min], 'k-', linewidth=boundary_width)  # Bottom wall
    ...
    if solver.stats()['success']:
        print("Solver succeeded!")
        relay_point_labeled = False
        waypoint_labeled = False
        for i in range(N-1):
            Ps=reeb_graph.nodes[Waypoints[i]].configuration
            if Flagb[i]!=0:
                if not relay_point_labeled:
                    ax.plot(Ps[0], Ps[1], 'ro', label='relay point')
                    relay_point_labeled = True
                else:
                    ax.plot(Ps[0], Ps[1], 'ro')
            else:
                if not waypoint_labeled:
                    ax.plot(Ps[0], Ps[1], 'go', label='waypoint')
                    waypoint_labeled = True
                else:
                    ax.plot(Ps[0], Ps[1], 'go')
            phi_new_opt=phi_opt[i]+Flagb[i]*np.pi/2 #A
            thetaC_opt=phi_opt[i+1]/2 + phi_new_opt/2
            sigma_opt=-1+ca.cos(phi_new_opt-phi_opt[i+1])
            l_opt=Distance[i]*(ca.cos(Angle[i]-phi_new_opt)-ca.cos(Angle[i]-phi_opt[i+1]))/sigma_opt
        ...
        ax.set_title('Planning Result')
        ax.set_aspect('equal')  # Keep x and y with same scale
        plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left')
        plt.tight_layout()
        print(f"🖼️  Saving figure to: {figure_file}")
