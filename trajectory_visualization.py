...
def plot_trajectory_with_time(waypoints, phi, r0, l, phi_new, time_segments, figure_file, reeb_graph, Flagb=None, case=None, N=None, show_plot=True):
    """
    ...
    for i in range(len(waypoints) - 1):
        ...
        if i < len(time_segments) and 'line' in time_segments[i] and len(time_segments[i]['line']) > 0:
            ...
            if line_length > 0.001:  # Only process meaningful lines
                ...
                # Check if this is a straight line (zero or very small radius)
                # After conversion, straight lines have r=0
                is_straight_line = abs(r0[i]) < 0.01  # Threshold for straight line (r≈0)

                if is_straight_line:
                    # For straight lines (r=0), line starts from waypoint position
                    # Use waypoint position directly, not x_after_arc
                    l_x = x_start  # Waypoint position
                    l_y = y_start  # Waypoint position

                    # Use phi_new[i] as direction (handles relay point rotation)
                    phi1_line = phi_new[i] if i < len(phi_new) else phi[i]
                else:
                    # For normal arcs, calculate line start point from arc center
    ...
    for i, (x, y, z, type_) in enumerate(zip(segment_markers_x, segment_markers_y, segment_markers_z, segment_types)):
        ...
        # Time labels removed as requested
    
    # Plot waypoints
    ax2.scatter(wp_x, wp_y, color='red', s=100, marker='o', label='Waypoints')
    
    # Add labels to waypoints with increased font size
    # for i, (x, y) in enumerate(zip(wp_x, wp_y)):
    #     ax2.text(x, y, f'WP{i}', fontsize=14)
    ...
    if relay_indices:
        ...
        # # Add labels to relay points with increased font size
        # for i, (x, y, idx) in enumerate(zip(relay_x, relay_y, relay_indices)):
        #     ax2.text(x, y, f'RP{idx}', fontsize=14)
    
    # Set axis labels and title with increased font size
    ax2.set_xlabel('X (meters)', fontsize=14)
    ax2.set_ylabel('Y (meters)', fontsize=14)
    ax2.set_title('2D Trajectory with Velocity Color Coding', fontsize=16)
    ...
    # Add a grid and legend with increased font size
    ax2.grid(True)
